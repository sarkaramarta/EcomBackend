...
export const getDashboardStats = TryCatch(async (req, res, next)=>{
    ...
    if(myCache.has(key)) stats = JSON.parse(myCache.get(key)!);
        ...
        const lastTransactionsPromise = Order.find({}).select(["orderItems", "discount", "total", "status"]).limit(4);
        ...
        const modifiedLatestTransaction = lastTransactions.map(i=>({
            _id:i._id,
            discount: i.discount,
            amount: i.total,
        ...
        stats = {
           ...
           userRatio,
           lastTransactions: modifiedLatestTransaction ,
        };
